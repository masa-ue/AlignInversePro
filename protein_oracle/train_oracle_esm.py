...
def main(args):
    ...
    print(len(dpo_train_dict), len(dpo_valid_dict_wt), len(dpo_test_dict_wt))
    if args.include_wt:
            ...
            for _, batch in tqdm(enumerate(loader_train)):
                ...
                if args.mixed_precision:
                    with torch.cuda.amp.autocast():
                        inputs = tokenizer(batch['aa_seq'], return_tensors="pt", padding=True, truncation=True).to(device)
                        outputs = model(**inputs)
                        dg_pred = outputs.logits.squeeze()
                        ...
                        if args.predict_ddg:
                            ...
                            if args.train_using_diff:
                                inputs_wt = tokenizer(batch['aa_seq_wt'], return_tensors="pt", padding=True, truncation=True).to(device)
                                outputs_wt = model(**inputs_wt)
                                dg_pred_wt = outputs_wt.logits.squeeze()
